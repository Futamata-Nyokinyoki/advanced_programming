...
int isMatchColor(Image *src, Image *templ, int x, int y)
{
    for (int j = 0; j < templ->height; j++)
    {
        for (int i = 0; i < templ->width; i++)
        {
            ...
            if (src->data[pt + 0] == 0 && src->data[pt + 1] == 0 && src->data[pt + 2] == 0)
            {
                continue;
            }
            else if (src->data[pt + 0] == 255 && src->data[pt + 1] == 255 && src->data[pt + 2] == 255)
            {
                continue;
            }
...
int level2(char *input_file, char *templ_file, int rotation, double threshold, char *options)
{
    CalcTime t;
    // 初期化
    t.start();
...
void process_image(char *image, char *level)
{
    ...
    for (unsigned int i = 0; i < glob_result.gl_pathc; i++)
    {
        ...
        if (x == 0)
        {
            level2(name, templ, rotation, 0.5, "cwp");
            x = 1;
        }
        else
        {
            level2(name, templ, rotation, 0.5, "wp");
        }
    }
